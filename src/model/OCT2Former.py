...
class OCT2Former(nn.Module):
    """
    OCT2Former: OCT-Inspired Transformer for Medical Image Segmentation
    
    """
    def __init__(self, n_channels=1, n_classes=2, embed_dims=[64, 128, 256], k=[128, 128, 128],
                 num_heads=[1, 2, 4], mlp_ratios=[4, 4, 4], depths=[1, 1, 1],
                 norm_layer=nn.LayerNorm, aux=False, spec_inter=False):
        super().__init__()
        ...
        if num_levels == 3:
            if spec_inter:
                self.up1 = up(embed_dims[2], size=(91, 91))
                self.up2 = up(embed_dims[1], size=(183, 183))
            else:
            ...
            if spec_inter:
                ...
                self.up1 = up(embed_dims[2])
                self.up2 = up(embed_dims[1], size=(91, 91))
            else:
    ...
    def get_model_info(self):
        return {
            'model_name': 'OCT2Former',
            'input_channels': self.n_channels,
            'output_channels': self.n_classes,
            'embed_dims': self.embed_dims,
            'num_levels': self.num_levels,
            'aux': self.aux,
            'description': 'OCT2Former: OCT-Inspired Transformer for Medical Image Segmentation'
        }
